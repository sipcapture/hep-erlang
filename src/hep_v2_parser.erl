...
%% @doc parses a complete HEPv2 packet into a HEPv3-like Erlang structure.
-spec parse(binary()) -> {ok, hep:state()} | {error, term(), binary()}.
parse(<<2:8, 28:8, 2:8, Protocol:8, SrcPort:16, DstPort:16, SrcIp:32, DstIp:32,
		Secs:32, USecs:32, CaptureId:16, _:16, Payload/binary>>) ->
	{ok, #hep{
		version = 2,
		chunks = [
			...
			{?HEP_GENERIC(?HEP_CAPTURE_AGENT_ID), <<0:16, CaptureId:16>>},
			{?HEP_GENERIC(?HEP_PAYLOAD), Payload}
		]
	}};
parse(<<2:8, 52:8, 10:8, Protocol:8, SrcPort:16, DstPort:16, SrcIp:128,
		DstIp:128, Secs:32, USecs:32, CaptureId:16, _:16, Payload/binary>>) ->
	{ok, #hep{
		version = 2,
		chunks = [
			{?HEP_GENERIC(?HEP_IP_PROTOCOL_FAMILY), <<2:8>>},
